...
const allInvalid = [invalid4, invalid5];
...
const idInvalidCardCompanies = nestedArray => {
  ...
  for (i = 0; i < nestedArray.length; i++) {
    if (firstDigits.includes(nestedArray[i][0])) {
      companyDigits.push(nestedArray[i][0]);
    }
  }
  if (companyDigits.includes(3)) {
    companiesToContact.push('Amex');
  }
  if (companyDigits.includes(4)) {
    companiesToContact.push('Visa');
  }
  if (companyDigits.includes(5)) {
    companiesToContact.push('Mastercard');
  }
  if (companyDigits.includes(6)) {
    companiesToContact.push('Discover');
  }
  if (companiesToContact.length > 0) {
